...
class Client():
  ...
  def res_mgmt_plots(self, well_id, refresh = None, refresh_abandonment_pressure = None,  start_date = None, end_date = None):
    logger.debug('Getting Well Mgmt Data')
    headers = self._get_headers()
    params = {}
    if refresh:
      params['refresh'] = refresh
    if refresh_abandonment_pressure:
      params['refresh_abandonment_pressure'] = refresh_abandonment_pressure
    ...
    if result.status_code == 200:
      ...
      raise Client_Exception('Unable to retrieve well mgmt Data')
